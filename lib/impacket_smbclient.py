...
import time
import logging
from impacket import smb, smb3, nt_errors
...
class MiniImpacketShell(cmd.Cmd):    
    def __init__(self, username = None, password = '', domain = '', hashes = None, host=None):
        ...
        # log in if we got the correct arguments passed
        if host is not None:
            self.smb = SMBConnection(host, host, sess_port=445)

            if username is not None:

                self.smb.login(username, password, domain=self.domain)
